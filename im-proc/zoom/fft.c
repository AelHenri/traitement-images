...
F cd(B *a,A *b,C rows,C cols){
  C c=rows, d=cols;B *aa=a;A *bb=b;D h=1.0/(c*d);
  for(C e=0;e<c;e++)for(C f=0;f<d;f++){
      D g=h*creal(*aa++);g=(e+f)%2?-g:g;g=g<0?0:g;g=g>M?M:g;*bb++=g;   
    }
}  
F ef(A *a,B *b,C c,C d){ab(a, b, c, d);}
F gh(B *a,A *b,C c,C d){cd(a, b, c, d);}
